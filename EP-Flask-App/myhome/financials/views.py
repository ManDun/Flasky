...
def add():
    ...
    if current_user.is_authenticated:
        print(f'User authenticated {current_user.username} and accessed add in expenses')
        form = AddExpense()
        ...
        print('User tried accessing financials.add and not authorized')
...
def dashboard():
    ...
    if current_user.is_authenticated:
        print(f'User authenticated {current_user.username} and accessed dashboard in expenses')
        form = SearchExpenses()
        user_id = current_user.id
        expenses = db.session.query(Expense, Files).outerjoin(Files, Expense.id==Files.expense_id)
        if request.method == 'GET':
        ...
        print('User tried accessing blogs.dashboard and not authorized')
        return redirect(url_for('auth.login'))
...
def delete(expense_id):
    ...
    if current_user.is_authenticated:
        print(f'User authenticated {current_user.username} and accessed delete in expenses')
